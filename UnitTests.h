...
TEST(UtilsTest, locate) {
	...
	auto loc = locate<string, CLArgBase>("-b", v, [](const CLArgBase &arg) {
		return arg.getOption();
	});
	ASSERT_TRUE(loc != v.cend());
	EXPECT_EQ(arg2, *loc);
	loc = locate<string, CLArgBase>("-e", v, [](const CLArgBase &arg) {
		return arg.getOption();
	});
	EXPECT_TRUE(loc == v.cend());
	// Test the long option field
	loc = locate<string, CLArgBase>("--bbb", v, [](const CLArgBase &arg) {
		return arg.getLongOption();
	});
	ASSERT_TRUE(loc != v.cend());
	EXPECT_EQ(arg2, *loc);
	loc = locate<string, CLArgBase>("--eee", v, [](const CLArgBase &arg) {
		return arg.getLongOption();
	});
	EXPECT_TRUE(loc == v.cend());
}
