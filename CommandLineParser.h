...
std::map<CLTYPE, std::string> CLTYPEtoStr { { CLTYPE::INT, "integer" }, { CLTYPE::FLOAT, "float" }, { CLTYPE::BOOL,
		"boolean" }, { CLTYPE::QUOTED, "quoted_string" }, { CLTYPE::STRING, "string" }, { CLTYPE::NONE, "" } };

// Define and represents a Command Line Argument (or option) without any value
class CLArgBase {
public:
	explicit CLArgBase(std::string opt = "", std::string longOpt = "", CLTYPE clt = CLTYPE::STRING, bool ismandatory =
			false, std::string docum = "") :
			option(std::move(opt)), longOption(std::move(longOpt)), cltype(clt), mandatory(ismandatory), doc(
					std::move(docum)) {
	}
	;
...
std::string CLArgBase::to_string() const {
	...
	os << "option=" << option << " longOption=" << longOption << " cltype=" << CLTYPEtoStr[cltype] << " mandatory="
			<< std::boolalpha << mandatory << " present=" << std::boolalpha << present;
	return os.str();
}
...
std::string usage(const std::vector<CLArgBase*> &options) {
	...
	auto maxLongOpt = (*maxSizeLongOption)->getLongOption().size() + 1;
	auto maxSizeOption = max_element(options.cbegin(), options.cend(), compOptionSize);
	auto maxOpt = (*maxSizeOption)->getOption().size() + 1;
	...
	for (const CLArgBase *o : options) // Display each option with its details
		os << "\t" << std::setw(maxOpt) << std::left << o->getOption() << ", " << std::setw(maxLongOpt) << std::left
				<< o->getLongOption() << (o->isMandatory() ? " [M] " : " [O] ") << std::setw(maxTyp) << std::left
				<< CLTYPEtoStr[o->getType()] << o->getDoc() << std::endl;
	return os.str();
}
